^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Changelog for package husky_pqm
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
0.6.1 (2022-01-18)
------------------
* [husky_pqm] Fixed roslaunch tests.
* Contributors: Tony Baltovski
...
* description.launch should be launched only once when the flag multimaster is false.
  ...
  Also delete the urdf subdirectory from the installation list of the package husky_pqm
  since the directory doesn't exist.
  HOW BUILT
  $ catkin_make_isolated --install --use-ninja
  HOW VERIFIED
  $ roslaunch husky_pqm husky_playpen.launch
...
0.2.5 (2015-12-31)
------------------
* Removed duplicate SICK laser plugin in husky_pqm, since husky_description already contains a SICK laser plugin from the lms package.
